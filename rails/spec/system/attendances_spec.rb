...
RSpec.describe "Attendances", type: :system do
    ...
    context 'ログインしている場合' do
      ...
      context '日付が今日ではない場合' do
        ...
        it '勤怠詳細ページが表示されるが、出勤・退勤ボタンが表示されない' do
          expect(page).to_not have_button '出勤'
          expect(page).to_not have_button '退勤'
          expect(page).to have_title("勤怠管理")
        end
      end
    end
  ...
  describe '出勤' do
    let!(:attendance) { create(:attendance, date: Time.current, clock_in_time: nil, clock_out_time: nil, user: user) }
    ...
    context '既に出勤している場合' do
      let!(:attendance) { create(:attendance, date: Time.current, clock_out_time: nil, user: user) }
  ...
  describe '退勤' do
    ...
    context '出勤して既に退勤している場合' do
      let!(:attendance) { create(:attendance, date: Time.current, user: user) }
